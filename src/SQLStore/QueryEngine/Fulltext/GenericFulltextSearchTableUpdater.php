...
class GenericFulltextSearchTableUpdater extends FulltextSearchTableUpdater {
	...
	public function addUpdatesFromPropertyTableDiff( CompositePropertyTableDiffIterator $compositePropertyTableDiffIterator ) {
		...
		foreach ( $diff as $table => $contents ) {
			...
			foreach ( $contents as $type => $content ) {
				...
				$this->doAggregateValuesFor( $type, $pid, $content, $deletes, $inserts );
			}
		}
	...
	private function doAggregateValuesFor( $type, $pid, array $content, array &$deletes, array &$inserts ) {
	...
	private function doRebuildTableRows( $table, $pid, $rows ) {
		...
		foreach ( $rows as $row ) {
			...
			$text = $row->o_blob === null ? $row->o_hash : $row->o_blob;

			// Only delete once for the table
			if ( !isset( $deleteCache[$sid . ':' . $pid] ) ) {
			//	$this->delete( $sid, $pid );
				$this->insert( $sid, $pid, '' );
			} else {
				$text = $this->read( $sid, $pid ) . ' ' . $text;
			}
